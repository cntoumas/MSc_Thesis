    ...
    begin
      case (block_clz)
        ...
        begin
          ...
          // Convergent rounding (round-half-to-even): add (bit0 AND bit1)
          //   - bit0 is the bit being dropped
          //   - bit1 is the new LSB
          // Adds 1 only when fraction is exactly 0.5 AND new LSB is 1,
          // flipping it to 0 (even). Removes the -0.5 LSB systematic bias
          // of plain truncation that produces visible spurs across bins.
          a_out_re <= a_in_re[INPUT_WIDTH-1 : 1] + (a_in_re[0] & a_in_re[1]);
          a_out_im <= a_in_im[INPUT_WIDTH-1 : 1] + (a_in_im[0] & a_in_im[1]);
          b_out_re <= b_in_re[INPUT_WIDTH-1 : 1] + (b_in_re[0] & b_in_re[1]);
          b_out_im <= b_in_im[INPUT_WIDTH-1 : 1] + (b_in_im[0] & b_in_im[1]);
