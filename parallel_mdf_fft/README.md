...
Implemented end-to-end with **Vivado** targeting `xc7a200tfbg676-2` (Artix-7, mid-size 200T device). Yosys 0.56 (`synth_xilinx`) is also supported for open-source area estimation.

### Vivado Post-Route Utilization (xc7a200tfbg676-2)

| Resource | Used | Available | Util % | Notes |
|---|---:|---:|---:|---|
| Slice LUTs       | **10,101** | 134,600 | 7.55 % | 7,686 as logic + 2,415 as memory |
...
| Static  | 0.133 W |
| Dynamic | 0.685 W |
| **Total on-chip** | **0.824 W** (~824 mW @ 100 MHz) |
...
### Comparison: Serial vs. Parallel (Vivado, xc7a200t)

| Resource | Serial FFT | Parallel MDF FFT | Ratio |
|---|---:|---:|---:|
| LUTs (total) | 2,372 | 10,101 | **4.3× more in Parallel** |
...
| Total power | 0.178 W | 0.824 W | **4.6× more in Parallel** |
...
| Resource | iCE40 (Yosys + nextpnr-ice40) | xc7a200t (Vivado) |
|---|---:|---:|
...
| **Total first-output latency** | **~543 super-clocks** | Analytical (see breakdown below) |
| **Total latency @ 100 MHz** | **~5.4 μs** | ≈ 543 cycles × 10 ns |
...
The Vivado project lives at `../MDF FFT/MDF FFT.xpr`. Open in Vivado, run `Implementation` → `Generate Bitstream`. Top module is `fft_axi_top`, target part `xc7a200tfbg676-2`, clock constraint in [constraints/fft_axi_top.xdc](constraints/fft_axi_top.xdc).
